//! App hardening (packer) scanner. Packers leave fingerprints both in the
//! Java loader class and in the native loader library.
...
pub fn scan<R: Read + Seek>(
    ...
    let rules: Vec<_> = sigs.by_category(Category::AppHardening)
        .iter()
        .map(|&i| &sigs.rules()[i])
        .collect();
